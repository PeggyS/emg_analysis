function [min_beg_t, min_end_t] = emg_analysis_find_max_emg(app, emg, beg_t, end_t, interval)
% emg is struct with .xdata and .ydata
% beg_t and end_t are the time interval (xdata) in which to look for max
% emg interval 

max_mean_over_interval = 0;
...
for cnt = beg_ind:end_ind-n_pts
	...
	% keep the largest interval mean
	if mean_over_interval > max_mean_over_interval
		max_mean_over_interval = mean_over_interval;
